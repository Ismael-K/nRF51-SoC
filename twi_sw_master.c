...
bool twi_master_transfer(uint8_t address, uint8_t * data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    {
        /* Transfer direction is from Slave to Master . AKA master recieving data */
...
{
#if 0
    if (TWI_SCL_READ() == 1 && TWI_SDA_READ() == 1) // reading the current state of the lines. If they're both high pull down SDA
    {
        // Pull SDA low
        TWI_SDA_LOW();
    }
    ...
    TWI_DELAY();
    TWI_SCL_LOW();
#endif // the previous if statements are making sure all conditions are covered
...
static bool twi_master_clock_byte(uint_fast8_t databyte)
{
    ...
    // MSB first
    for (uint_fast8_t i = 0x80; i != 0; i >>= 1) // shifting through each binary 
    {
        TWI_SCL_LOW();
        TWI_DELAY();
...
static bool twi_master_clock_byte_in(uint8_t *databyte, bool ack)
{
    ...
    *databyte = (uint8_t)byte_read; //what ever databyte is pointing to , piut byte_read in there
