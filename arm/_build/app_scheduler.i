...
#line 15 "app_scheduler.c"
#line 16 "app_scheduler.c"
                             ...
                             app_sched_event_handler_t handler)
{
    ...
    if (event_data_size <= m_queue_event_size)
    {
        ...
        { uint8_t IS_NESTED_CRITICAL_REGION = 0; uint32_t CURRENT_INT_PRI = current_int_priority_get(); if (CURRENT_INT_PRI != APP_IRQ_PRIORITY_HIGH) { uint32_t ERR_CODE = sd_nvic_critical_region_enter(&IS_NESTED_CRITICAL_REGION); if (ERR_CODE == ((0x0) + 2)) { __disable_irq(); } else { do { const uint32_t LOCAL_ERR_CODE = (ERR_CODE); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 89, (uint8_t*) "app_scheduler.c"); } while (0); } } while (0); } };
        ...
        if (CURRENT_INT_PRI != APP_IRQ_PRIORITY_HIGH) { uint32_t ERR_CODE; __enable_irq(); ERR_CODE = sd_nvic_critical_region_exit(IS_NESTED_CRITICAL_REGION); if (ERR_CODE != ((0x0) + 2)) { do { const uint32_t LOCAL_ERR_CODE = (ERR_CODE); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 97, (uint8_t*) "app_scheduler.c"); } while (0); } } while (0); } } };
