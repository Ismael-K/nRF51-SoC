...
uint32_t ble_lbs_init(ble_lbs_t * p_lbs, const ble_lbs_init_t * p_lbs_init)
{
    ...
    if (err_code != ((0x0) + 0))
    {
		...
		err_code = user_input_write_char_add(p_lbs); 
		if (err_code != ((0x0) + 0))
    {
        return err_code;
    }
		err_code = image_char_add(p_lbs); 
		if (err_code != ((0x0) + 0))
    {
        return err_code;
    }
...
uint32_t ble_send_image(ble_lbs_t * p_lbs, uint8_t *data, uint16_t length){ 
  ...
  uint16_t len = sizeof(data);
	...
	memset(&params, 0, sizeof(params));
  params.handle = p_lbs->image_char_handles.value_handle; 
	params.p_data = data; 
  params.p_len = &len; 
  params.type = 0x01;
