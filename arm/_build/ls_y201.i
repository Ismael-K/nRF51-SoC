...
uint16_t JPEG_address = 0x0000;						
volatile uint16_t JPEG_file_size;					
uint8_t *p;
int *temp;
...
void LSY201_FileSize(){						
	...
	RX_ACK[7] = simple_uart_get();			
	RX_ACK[8] = simple_uart_get();				
	JPEG_file_size = 0x100*RX_ACK[7] + RX_ACK[8];	
...
void LSY201_ReadFile(uint16_t LSY201_ChunkBytes){	
	...
	uint8_t MH = JPEG_address/0x100;		
	uint8_t ML = JPEG_address%0x100;		
	uint8_t KH = LSY201_ChunkBytes/0x100;	
	uint8_t KL = LSY201_ChunkBytes%0x100;	
	...
	JPEG_address += LSY201_ChunkBytes;		
...
_Bool Get_Chunk_JPEG(uint16_t BytesPerChunk, uint8_t arr[]){
	...
	LSY201_ReadFile(BytesPerChunk);		
	Skip_ack();
	for(rj2=0; rj2<BytesPerChunk; rj2++){
		arr[rj2] = simple_uart_get();	

	}
	Skip_ack();
	if(NumOfBytesRemain > 0){
		LSY201_ReadFile(NumOfBytesRemain);			
		Skip_ack();
		for(rj3=0; rj3<NumOfBytesRemain; rj3++){
			arr[rj3] = simple_uart_get(); 
...
void Get_Full_JPEG(uint16_t BytesPerChunk){
	...
	p = (uint8_t *)calloc(JPEG_file_size,sizeof(uint8_t));  
	...
	{	
		...
		Skip_ack();
		for(rj2=0; rj2<BytesPerChunk; rj2++){
			p[rj1*BytesPerChunk + rj2] = simple_uart_get();	
	...
	if(NumOfBytesRemain > 0){
		...
		Skip_ack();
		for(rj3=0; rj3<NumOfBytesRemain; rj3++){
			p[(NumOfFullChunks*BytesPerChunk) + rj3] = simple_uart_get(); 
...
void LSY201_Resolution(uint8_t LSY201_Res){	
	...
	if(LSY201_Res == 1)						
		XX = 0x00;
	else if(LSY201_Res == 2)				
		XX = 0x11;
	else if(LSY201_Res == 3)				
		XX = 0x22;
